<script setup lang="ts">
  ...
  const initCanvas = async (canvasEl: HTMLCanvasElement) => {
    ...
    canvas.value.on('mouse:down', (opt: any) => {
      ...
      if (isCropping.value) {
        ...
        // 如果点击的不是“裁剪框”本身，说明点击了外部 -> 确认裁剪
        if (target !== cropZone.value) {
          confirmCrop()
        }
        return // 🚨 关键：处理完剪辑逻辑后，直接退出，不执行后面代码
      }
      ...
      if (isPenMode.value) {
        ...
        if (penPoints.length === 0) {
          penPoints.push(pointer)

          // 创建一根“跟随鼠标”的预览线 (Active Line)
          const points = [pointer.x, pointer.y, pointer.x, pointer.y]
          activeLine = new fabric.value.Line(points, {
            strokeWidth: 2,
            stroke: '#999999',
            class: 'line',
            originX: 'center',
          ...
          // 把 activeLine 变成实线并固定下来 (存入 penLines)
          activeLine.set({ strokeDashArray: null, stroke: '#333333' })
          penLines.push(activeLine)

          // 创建新的 activeLine 用于下一次预览
          const points = [pointer.x, pointer.y, pointer.x, pointer.y]
          activeLine = new fabric.value.Line(points, {
            strokeWidth: 2,
            stroke: '#999999',
        ...
        return // 🚨 同样：处理完钢笔逻辑后退出
      }

      // =================================================
      // 场景 3: 普通模式 (如果有其他点击逻辑写在这里)
      // =================================================
      // 例如：点击空白处取消选中 (Fabric 默认会做，这里可以处理额外的 UI 逻辑)
    })

    // 2. 鼠标移动：更新预览线
    canvas.value.on('mouse:move', (opt: any) => {
      if (!isPenMode.value || !activeLine) return
  ...
  */
  ...
  let penPoints: { x: number; y: number }[] = [] // 记录点击的所有坐标点
  let penLines: any[] = [] // 记录画出来的辅助线
  let activeLine: any = null // 跟随鼠标移动的那根线
  ...
  const finishPenDrawing = () => {
    ...
    // 1. 创建多边形对象
    const polygon = new fabric.value.Polygon(penPoints, {
      fill: '#cccccc', // 默认灰色填充
      stroke: '#333333', // 深色描边
      strokeWidth: 2,
      objectCaching: false,
      left: 0, // 暂时设为0，后面用 centerObject 修正
      top: 0
    })

    // 2. 修正坐标 (Fabric 的 Polygon 创建后坐标会偏移，需要重新定位)
    // 这一步非常关键，否则图形会乱飞
    // 我们先把它加进去，然后让它自适应位置
    // (Fabric v4/v5/v6 创建 Polygon 时会自动计算边界框)
    // 但为了保险，我们手动计算一下中心点
